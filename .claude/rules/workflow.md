...
- If new files are created, or any changes to the project directory structure are made, document in `/Docs/project_structure.md`
- If there are any changes to the backend setup, document in `/DATABASE_SCHEMA.md`
- If there are any changes to the UI UX philosophy or user journeys, document in `/Docs/UI_UX_doc.md`
- If there are any changes to the product vision, goals, or scope, document in `PRD.md`
- **Check for outdated documentation**: Before marking any task complete, review existing bug entries in `/Docs/Bug_tracking.md` to ensure they accurately reflect current implementation. Update status (e.g., "Superseded by BUG-XXX") if the implementation has changed significantly from what was originally documented.

### File Reference Priority
1. `/Docs/Bug_tracking.md` - Check for known issues first
2. `/Docs/Implementation.md` - Main task reference
3. `/Docs/project_structure.md` - Structure guidance
4. `/DATABASE_SCHEMA.md` - Backend setup information
...
4. **User Verification**: Prompt user to test, and wait for user to confirm functionality works
